...
// import SideMenu from '../components/SideMenu'; // 1. REMOVEMOS A IMPORTAÇÃO
...
function RelatorioPage() {
  ...
  const getFaturamentoCardTitle = () => {
    ...
    if (reportPeriod === 'daily') return `Faturamento do Dia (${dateForTitle.toLocaleDateString('pt-BR', { timeZone: 'UTC' })})`;
    ...
    if (reportPeriod === 'monthly') return `Faturamento de ${new Date(Date.UTC(selectedYear, selectedMonth - 1)).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric', timeZone: 'UTC' })}`;
    if (reportPeriod === 'yearly') return `Faturamento de ${selectedYear}`;
  };
  ...
  return (
    // <>  <-- Não precisamos mais do fragmento extra
      // <SideMenu /> // 2. REMOVEMOS O COMPONENTE DAQUI
